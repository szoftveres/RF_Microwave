...
for element = [1 2]
    ...
    % Slot
    start = [-(slot_length/2 + x_separation) -slot_width/2 _z_offset_];
    ...
    patch_width = 11.2; %11.2              % Y the resonant length

    start = [-(patch_length/2 + x_separation) -patch_width/2 substrate.thickness_1 + _z_offset_ + stack_thickness + gap];
    ...
    start = [-(patch_length/2 + x_separation) -patch_width/2 substrate.thickness_1 + _z_offset_];
    stop = start + [patch_length patch_width 0];
    CSX = AddBox( CSX, 'copper', 20,  start, stop);
    ...
    start = [-(feedline_width/2 + x_separation) -substrate.y/2 -substrate.thickness_2+_z_offset_];
    ...
    if (element == 1)
        [CSX port{1}] = AddLumpedPort(CSX, 5 ,1 ,feed.R, start, stop, [0 0 1], true);  % port field: Z direction
    end
    if (element == 2) 
        [CSX port{2}] = AddLumpedPort(CSX, 5 ,2 ,feed.R, start, stop, [0 0 1], false);  % port field: Z direction
    end
end
